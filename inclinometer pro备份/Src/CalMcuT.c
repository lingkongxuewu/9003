...
{
     McuT_SetCalCnt(&McuTcal,cnt);    //设置X轴温度补偿点个数
}
...
{
	McuTcal.ptempadc=mtemp_buf;            //温度补偿模块中温度AD值缓存数组
...
{
    int8u i;
    SetMcuTCalCnt(5);          								//设置温度补偿点个数
    for(i=0;i<MTEMP_CAL_NUM;i++)								//21
    {
        McuTcal.ptempadc[i] = 80*(i-MTEMP_CAL_NUM/2)+11200; //MCU的温度AD
		if(i==0)
        {
...
float GetMt(int16u tadc)
{
    ...
    {
        if(tadc<(PMCUTCAL->ptempadc[i+1]))    						//查找温度在温补模块中温度数组中的位置
            break;
    }
    offset = (PMCUTCAL->prm_k[i]*tadc+PMCUTCAL->prm_a[i])*1000;    //获取相应的K,A值，并计算
